...
def test_target_patch_approve_reject_and_apply_routes(tmp_path: Path) -> None:
    ...
    try:
        with TestClient(app) as client:
            ...
            second_trigger_response = client.post(
                "/trigger",
                json={
                    "source_type": "webhook",
                    "payload": {
                        "source_name": "local-webhook",
                        "changed_files": ["backend/src/ariadne_doc_assistant/api/routes.py"],
                        "summary": "Another API route change",
                    },
                    "context": {"component": "backend"},
                },
            )
            rejected_patch_id = second_trigger_response.json()["patch"]["id"]
            reject_response = client.post(f"/patches/{rejected_patch_id}/reject")
    finally:
        app.dependency_overrides.clear()
